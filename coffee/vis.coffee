...
Network = () ->
  ...
  #height = 750
  width = 685
  height = 750  # allData will store the unfiltered data
  allData = []
  curLinksData = []
  curNodesData = []
  linkedByIndex = {}
  ...
  showDetails = (d,i) ->
    ...
    content += '<a class="addlink" ontouchend="appendAudioIpad(\'' + name + '\', \'' + artist + '\', \'' + mp3_url + '\');\" ontouchend="playAudio(\'' + name + '\', \'' + artist + '\');\">' + name + '</a></p>'
  ...
  addtoArray = (d,i) ->
    ...
    track = $(this).text;
    elements.push(track);
    printout = elements.toString();
    document.write (printout)
    
  # Add item to playlist GOES HERE
  addItem = (d,i) ->
    mp3_url = d.mp3_url
    name = d.name
    artist = d.artist
        
    if (d.mp3_url == undefined) || (d.mp3_url == "No result") 
        brokenAudio(name,artist);

    else        
        appendAudio(name,artist,mp3_url);
...
$ ->
  ...
  d3.json "data/fake_trees.json", (json) ->
    myNetwork("#vis", json)
